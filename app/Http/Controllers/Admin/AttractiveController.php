...
class AttractiveController extends Controller
{
    ...
    public function getHome($status){
        switch ($status) {
            case '0':
                $attractives = Attractive::with(['cat'])->where('status', '0')->orderBy('id', 'desc')->paginate(10); 
                break;
            case '1':
                $attractives = Attractive::with(['cat'])->where('status', '1')->orderBy('id', 'desc')->paginate(10);
                break;
            case 'all':
                $attractives = Attractive::with(['cat'])->orderBy('id', 'desc')->paginate(10);
                break;
            case 'trash':
                $attractives = Attractive::with(['cat'])->onlyTrashed()->orderBy('id', 'desc')->paginate(10);
                break;
        }
    ...
    public function getAttractiveAdd(){
        $cats = Category::where('module', '0')->where('module', '0')->pluck('name', 'id');
        $data = ['cats' => $cats];
    ...
    public function postAttractiveAdd(Request $request){
        ...
        if($validator->fails()):
            ...
            if($attractive->save()):
                ...
                return redirect('/admin/attractives')->with('message', 'Guardado con éxito.')->with(
                'typealert', 'success');
            endif; 
        endif; 
    }
