...
function editor_init(field){
	CKEDITOR.replace(field,{
		...
		{name: 'document', items: [ 'CodeSnippet', 'EmojiPanel', 'Preview', 'Sourse']}
...
function delete_object(e){
	...
	var title, text, icon;
	if(action == "delete"){
		title = "¿Estas seguro de eliminar este Atractivo?";
		text = "Recuerda que esta acción enviara este elemento a la papelera o lo eliminara de forma definitiva!";
		icon = "warning";
	}
	if(action == "restore"){
		title = "¿Quieres restaurar este Atractivo?";
		text = "Esta acción restaurará este Atractivo y estará activo en la base de datos";
		icon = "info";
	}
	Swal.fire({
  		title: title,
